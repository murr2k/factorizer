/**
 * CUDA Factorizer Version 2.2.0 - Unified Integration Master
...
__global__ void trial_division_kernel(
    ...
    for (int d = start; d < end && *factor_count < 64; d++) {
        ...
        uint128_t remainder;
        uint128_t quotient = divide_128(n, divisor, remainder);
...
__global__ void pollards_rho_unified(
    ...
    // Setup reduction based on configuration
    Barrett128_v2* barrett = nullptr;
    Montgomery128* montgomery = nullptr;
    
    if (config.use_barrett) {
        barrett = new Barrett128_v2(n);
    }
    if (config.use_montgomery && (n.low & 1)) {
        montgomery = new Montgomery128(n);
    }
    ...
    for (int i = 0; i < config.max_iterations && !progress->factor_found; i++) {
        ...
        if (config.type == AlgorithmType::POLLARDS_RHO_BRENT) {
            ...
            for (int j = 0; j < m && i + j < r; j++) {
                y = pollards_f(y, c, n, barrett, montgomery);
                uint128_t diff = (y > ys) ? subtract_128(y, ys) : subtract_128(ys, y);
                
                if (montgomery) {
                    uint128_t mont_diff = to_montgomery(diff, *montgomery);
                    product = montgomery_multiply(product, mont_diff, *montgomery);
                } else {
            ...
            // Basic variant
            x = pollards_f(x, c, n, barrett, montgomery);
            y = pollards_f(pollards_f(y, c, n, barrett, montgomery), c, n, barrett, montgomery);
    ...
    // Cleanup
    if (barrett) delete barrett;
    if (montgomery) delete montgomery;
}

// Helper function for Pollard's f
__device__ uint128_t pollards_f(
    const uint128_t& x,
    const uint128_t& c,
    const uint128_t& n,
    Barrett128_v2* barrett,
    Montgomery128* montgomery
) {
    uint256_t x_squared = multiply_128_128(x, x);
    uint128_t result;
    
    if (montgomery && (n.low & 1)) {
        result = montgomery_reduce(x_squared, *montgomery);
    } else if (barrett) {
        result = barrett->reduce(x_squared);
    } else {
        // Fallback to simple modulo
        uint128_t remainder;
        divide_256_128(x_squared, n, remainder);
        result = remainder;
    }
...
private:
    ...
    bool run_pollards_rho(uint128_t n, const AlgorithmConfig& algo_config) {
        ...
        // Final check
        cudaMemcpy(&factor_count, d_factor_count, sizeof(int), cudaMemcpyDeviceToHost);
        result.total_iterations = progress_reporter ? 
            progress_reporter->get_total_iterations() : 0;
